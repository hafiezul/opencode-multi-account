...
test("project config keeps nearer files last across mixed extensions", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      await Filesystem.write(
      ...
      await Filesystem.write(
        path.join(child, "opencode.jsonc"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
          model: "child-jsonc",
        }),
  ...
  await Instance.provide({
    directory: path.join(tmp.path, "child"),
    fn: async () => {
      const config = await Config.get()
      expect(config.model).toBe("child-jsonc")
    },
  })
})
...
test("nearer .opencode config overrides ancestor .opencode config", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const root = path.join(dir, ".opencode")
      const child = path.join(dir, "child")
      const local = path.join(child, ".opencode")
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await Config.get()
      expect(config.model).toBe("child-opencode")
    },
  })
})
...
test.serial("config reloads after auth changes and clears stale wellknown env", async () => {
  const originalFetch = globalThis.fetch
  const key = "TEST_WELLKNOWN_TOKEN"
  const prev = process.env[key]
  const mockFetch = mock((url: string | URL | Request) => {
    const urlStr = url.toString()
    if (urlStr === "https://one.example.com/.well-known/opencode") {
      return Promise.resolve(
        new Response(
          JSON.stringify({
            config: {
    ...
    if (urlStr === "https://two.example.com/.well-known/opencode") {
      return Promise.resolve(
        new Response(
          JSON.stringify({
            config: {
  ...
  try {
    await Auth.remove("https://one.example.com")
    await Auth.remove("https://two.example.com")
    ...
    await Auth.set("https://one.example.com", {
      type: "wellknown",
      key,
      token: "one-token",
    })
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        expect((await Config.get()).username).toBe("one")
        expect(Env.get(key)).toBe("one-token")
        expect(process.env[key]).toBeUndefined()

        await Auth.remove("https://one.example.com")
        await Auth.set("https://two.example.com", {
          type: "wellknown",
          key,
          token: "two-token",
        })

        expect((await Config.get()).username).toBe("two")
        expect(Env.get(key)).toBe("two-token")
        expect(process.env[key]).toBeUndefined()
        ...
        const config = await Config.get()
        expect(config.username).toBeDefined()
        expect(config.username).not.toBe("two")
...
test.serial("auth cleanup restores pre-existing env values", async () => {
  const originalFetch = globalThis.fetch
  const key = "TEST_WELLKNOWN_RESTORE"
  const prev = process.env[key]
  process.env[key] = "user-token"

  globalThis.fetch = mock((url: string | URL | Request) => {
    const urlStr = url.toString()
    if (urlStr === "https://restore.example.com/.well-known/opencode") {
      return Promise.resolve(
        new Response(
          JSON.stringify({
            config: {
  ...
  try {
    await Auth.remove("https://restore.example.com")
    ...
    await Auth.set("https://restore.example.com", {
      type: "wellknown",
      key,
      token: "auth-token",
    })
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        expect((await Config.get()).username).toBe("restore")
        expect(process.env[key]).toBe("user-token")
        expect(Env.get(key)).toBe("auth-token")

        await Auth.remove("https://restore.example.com")

        await Config.get()
        expect(process.env[key]).toBe("user-token")
        expect(Env.get(key)).toBe("user-token")
      },
    })
  } finally {
    await Auth.remove("https://restore.example.com")
...
test.serial("config cleanup restores env values on instance disposal", async () => {
  const originalFetch = globalThis.fetch
  const key = "TEST_WELLKNOWN_DISPOSE"
  const prev = process.env[key]
  delete process.env[key]

  globalThis.fetch = mock((url: string | URL | Request) => {
    const urlStr = url.toString()
    if (urlStr === "https://dispose.example.com/.well-known/opencode") {
      return Promise.resolve(
        new Response(
          JSON.stringify({
            config: {
  ...
  try {
    await Auth.remove("https://dispose.example.com")
    await Auth.set("https://dispose.example.com", {
      type: "wellknown",
      key,
      token: "dispose-token",
    })
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        expect((await Config.get()).username).toBe("dispose")
        expect(process.env[key]).toBeUndefined()
        expect(Env.get(key)).toBe("dispose-token")
...
test.serial("config reloads when active account or org changes", async () => {
  const originalActive = Account.active
  const originalConfig = Account.config
  const originalToken = Account.token
  const prev = process.env["OPENCODE_CONSOLE_TOKEN"]
  let n = 0
  ...
  Account.config = mock(async (accountID, orgID) => ({
    username: `${accountID}:${orgID}`,
  }))

  Account.token = mock(async (accountID) => AccessToken.make(`token:${accountID}`))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(first.username).toBe("account-1:org-1")
        expect(Env.get("OPENCODE_CONSOLE_TOKEN")).toBe("token:account-1")
        expect(process.env["OPENCODE_CONSOLE_TOKEN"]).toBe(prev)

        n = 1

        const second = await Config.get()
        expect(second.username).toBe("account-2:org-2")
        expect(Env.get("OPENCODE_CONSOLE_TOKEN")).toBe("token:account-2")
        expect(process.env["OPENCODE_CONSOLE_TOKEN"]).toBe(prev)
      },
    })
  } finally {
    ...
    if (prev === undefined) {
      ...
      process.env["OPENCODE_CONSOLE_TOKEN"] = prev
    }
  }
})
