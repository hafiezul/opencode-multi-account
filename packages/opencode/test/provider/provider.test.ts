...
import { Account, AccountID, OrgID } from "../../src/account"
...
test("active profile switch updates Provider.list in the same instance", async () => {
  await using tmp = await tmpdir({
    config: {
      provider: {
        "switch-list": {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      await Auth.put("switch-list", "work", { type: "api", key: "work-key" })
      await Auth.put("switch-list", "personal", { type: "api", key: "personal-key" })
      await Auth.activate("switch-list", "work")
      expect((await Provider.list())[ProviderID.make("switch-list")].key).toBe("work-key")
      await Auth.activate("switch-list", "personal")
      expect((await Provider.list())[ProviderID.make("switch-list")].key).toBe("personal-key")
    },
  })
})

test.serial("active account switch updates Provider.list in the same instance", async () => {
  ...
  Account.config = mock(async (account, org) => ({
    provider: {
      "switch-account": {
        name: "Switch Account",
        npm: "@ai-sdk/openai-compatible",
  ...
  try {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        expect((await Provider.list())[ProviderID.make("switch-account")].options.apiKey).toBe("account-1:org-1")
        n = 1
        expect((await Provider.list())[ProviderID.make("switch-account")].options.apiKey).toBe("account-2:org-2")
      },
    })
  } finally {
...
test("getLanguage returns a different object after profile switch", async () => {
  await using tmp = await tmpdir({
    config: {
      provider: {
        "switch-language": {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      await Auth.put("switch-language", "work", { type: "api", key: "work-key" })
      await Auth.put("switch-language", "personal", { type: "api", key: "personal-key" })
      await Auth.activate("switch-language", "work")
      const model = await Provider.getModel(ProviderID.make("switch-language"), ModelID.make("model"))
      const a = await Provider.getLanguage(model)
      await Auth.activate("switch-language", "personal")
      const b = await Provider.getLanguage(model)
      expect(b).not.toBe(a)
    },
  })
})

test("provider availability refreshes when active auth is removed or restored", async () => {
  await using tmp = await tmpdir({
    config: {
      provider: {
        "switch-availability": {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      await Auth.put("switch-availability", "work", { type: "api", key: "work-key" })
      await Auth.activate("switch-availability", "work")
      expect((await Provider.list())[ProviderID.make("switch-availability")]).toBeDefined()
      await Auth.removeProfile("switch-availability", "work")
      expect((await Provider.list())[ProviderID.make("switch-availability")]).toBeUndefined()
      await Auth.put("switch-availability", "restored", { type: "api", key: "restored-key" })
      await Auth.activate("switch-availability", "restored")
      expect((await Provider.list())[ProviderID.make("switch-availability")]).toBeDefined()
    },
  })
})

test.serial("sap-ai-core switches auth without persisting process env state", async () => {
  ...
  try {
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        await Auth.put("sap-ai-core", "work", { type: "api", key: "work-key" })
        await Auth.activate("sap-ai-core", "work")
        const work = (await Provider.list())[ProviderID.make("sap-ai-core")]
        expect(work).toBeDefined()
        expect(work.options.serviceKey).toBe("work-key")
        expect(process.env.AICORE_SERVICE_KEY).toBeUndefined()

        await Auth.put("sap-ai-core", "personal", { type: "api", key: "personal-key" })
        await Auth.activate("sap-ai-core", "personal")
        const personal = (await Provider.list())[ProviderID.make("sap-ai-core")]
        expect(personal).toBeDefined()
        expect(personal.options.serviceKey).toBe("personal-key")
        expect(process.env.AICORE_SERVICE_KEY).toBeUndefined()

        await Auth.removeProfile("sap-ai-core", "personal")
        expect((await Provider.list())[ProviderID.make("sap-ai-core")]).toBeUndefined()
        expect(process.env.AICORE_SERVICE_KEY).toBeUndefined()

        process.env.AICORE_SERVICE_KEY = "user-key"
        await Auth.put("sap-ai-core", "restored", { type: "api", key: "restored-key" })
        await Auth.activate("sap-ai-core", "restored")
        const restored = (await Provider.list())[ProviderID.make("sap-ai-core")]
        expect(restored).toBeDefined()
        expect(restored.options.serviceKey).toBe("user-key")
        expect(process.env.AICORE_SERVICE_KEY).toBe("user-key")
      },
    })
  } finally {
    if (prev === undefined) delete process.env.AICORE_SERVICE_KEY
    else process.env.AICORE_SERVICE_KEY = prev
  }
})

test.serial("sap-ai-core scopes env override to sdk creation", async () => {
  const prev = process.env.AICORE_SERVICE_KEY
  delete process.env.AICORE_SERVICE_KEY

  await using tmp = await tmpdir({
    init: async (dir) => {
      const mod = path.join(dir, "sap-provider.js")
      await Bun.write(
        mod,
        [
          "export function createSap(opts) {",
          "  const env = process.env.AICORE_SERVICE_KEY",
          "  return (id) => ({ id, env, opts })",
          "}",
        ].join("\n"),
      )
      await Bun.write(
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const model = await Provider.getModel(
          ProviderID.make("sap-ai-core"),
          ModelID.make("anthropic--claude-4.5-opus"),
        )
        const lang = (await Provider.getLanguage(model)) as unknown as {
          env: string | undefined
          opts: Record<string, unknown>
        }
        expect(lang.env).toBe("work-key")
        expect(lang.opts.deploymentId).toBeUndefined()
        expect(process.env.AICORE_SERVICE_KEY).toBeUndefined()

        process.env.AICORE_SERVICE_KEY = "user-key"
        await Auth.put("sap-ai-core", "home", { type: "api", key: "home-key" })
        await Auth.activate("sap-ai-core", "home")
        const next = await Provider.getModel(ProviderID.make("sap-ai-core"), ModelID.make("anthropic--claude-4.5-opus"))
        const user = (await Provider.getLanguage(next)) as unknown as { env: string | undefined }
        expect(user.env).toBe("user-key")
        expect(process.env.AICORE_SERVICE_KEY).toBe("user-key")
      },
    })
  } finally {
...
test("getLanguage fails cleanly when a stale model loses its provider", async () => {
  await using tmp = await tmpdir({
    config: {
      provider: {
        "switch-stale": {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      await Auth.put("switch-stale", "work", { type: "api", key: "work-key" })
      await Auth.activate("switch-stale", "work")
      const model = await Provider.getModel(ProviderID.make("switch-stale"), ModelID.make("model"))
      await Provider.getLanguage(model)

      await Auth.removeProfile("switch-stale", "work")

      await expect(Provider.getLanguage(model)).rejects.toBeInstanceOf(Provider.ModelNotFoundError)
    },
  })
})
